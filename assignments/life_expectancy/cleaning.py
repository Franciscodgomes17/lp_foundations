...
def load_data(input_path: Path = Paths.input_file) -> pd.DataFrame:
    """
...
def clean_data(df: pd.DataFrame, country: str = "PT") -> pd.DataFrame:
    ...
    # Year: strip spaces then int
    long_df[Cols.YEAR] = long_df[Cols.YEAR].astype(str).str.strip().astype(int)

    # Value: ":" means missing; remove stray non-numeric chars (e.g., flags)
    cleaned_value = (
        long_df[Cols.VALUE]
        .astype(str)
        .str.strip()
...
def save_data(df: pd.DataFrame, output_path: Path = Paths.output_file) -> None:
    """
...
def main(country: str = "PT") -> None:
    """
    Orchestrates the full pipeline: load -> clean -> save.
    """
    raw = load_data()
    cleaned = clean_data(raw, country=country)
    save_data(cleaned)
